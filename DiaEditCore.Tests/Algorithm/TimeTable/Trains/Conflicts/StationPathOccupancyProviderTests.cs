namespace DiaEditCore.Tests.Algorithm.TimeTable.Trains.Conflicts;

using DiaEditCore.Algorithm.Stations.FloorUnitObjects;
using DiaEditCore.Algorithm.TimeTable.Trains.Conflicts;
...
public class StationPathOccupancyProviderTests
{
    ...
    {
        var (scs, segs, allMainRoutes, pathsById, arrivalIndex, departureIndex) = BuildTopology(arrivalAdjustmentSec: 30);
        var train = MakeArrivingTrain(1, arrivalSeconds: 1000, TrackB);

        var result = StationPathOccupancyProvider.BuildOccupancy([train], scs, segs, allMainRoutes, pathsById, arrivalIndex, departureIndex);
    ...
    {
        var (scs, segs, allMainRoutes, pathsById, arrivalIndex, departureIndex) = BuildTopology(departureAdjustmentSec: 20);
    ...
    {
        var (scs, segs, allMainRoutes, pathsById, arrivalIndex, departureIndex) = BuildTopology(arrivalAdjustmentSec: 30, departureAdjustmentSec: 20);
    ...
    public void IsProvisionalな仮列車も占有対象に含まれる()
    {
        ...
        var result = StationPathOccupancyProvider.BuildOccupancy([provisionalTrain], scs, segs, allMainRoutes, pathsById, arrivalIndex, departureIndex);
    ...
    public void 複数Trainが同一到着用StationPathを重複する時間帯で使用する場合ConflictCheckerで検出できる()
    {
        ...
        var result = StationPathOccupancyProvider.BuildOccupancy(trains, scs, segs, allMainRoutes, pathsById, arrivalIndex, departureIndex);
        var occ = Get(result, ArrivalPathId);
    ...
    public void RunSegmentsが空のTrainはスキップされ例外を投げない()
    {
        ...
        var exception = Record.Exception(() =>
            StationPathOccupancyProvider.BuildOccupancy([emptyTrain], scs, segs, allMainRoutes, pathsById, arrivalIndex, departureIndex));
