...
public class TrackOccupancyProviderTests
{
    ...
    ) BuildTopology(int arrivalAdjustmentSec = 30, int departureAdjustmentSec = 20)
    {
        ...
        return (scs, segs, pathsById, arrivalIndex, departureIndex, rails, shuntingPath);
    }
    ...
    public void PrevTrainNextTrainがともに解決できるとき双方の占有区間が接続点をまたいで意図的に重複する()
    {
        ...
        var (scs, segs, pathsById, arrivalIndex, departureIndex, rails, _) = BuildTopology();
        ...
        var result = TrackOccupancyProvider.BuildOccupancy(trains, scs, segs, pathsById, arrivalIndex, departureIndex, rails, settings);
    ...
    public void PrevTrainが存在しない始発列車はDiagramBasedTimeSecで占有開始が打ち切られる()
    {
        ...
        var result = TrackOccupancyProvider.BuildOccupancy([departing], scs, segs, pathsById, arrivalIndex, departureIndex, rails, settings);
    ...
    public void NextTrainが存在しない終着列車は自身の到着占有終了で打ち切られる()
    {
        ...
        var result = TrackOccupancyProvider.BuildOccupancy([arriving], scs, segs, pathsById, arrivalIndex, departureIndex, rails, settings);
    ...
    public void Shunting作業の占有区間はTrack占有に追加される()
    {
        ...
        train.StopTimes[new StopKey(StB, 1)].Works.Add(new StationWork
        {
        ...
        var result = TrackOccupancyProvider.BuildOccupancy([train], scs, segs, pathsById, arrivalIndex, departureIndex, rails, settings);
    ...
    public void RunSegmentsが空のTrainはスキップされ例外を投げない()
    {
        ...
        var exception = Record.Exception(() =>
            TrackOccupancyProvider.BuildOccupancy([emptyTrain], scs, segs, pathsById, arrivalIndex, departureIndex, rails, settings));
